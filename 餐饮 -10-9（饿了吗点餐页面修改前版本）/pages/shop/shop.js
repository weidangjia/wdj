...
Page({
  ...
  onLoad: function (options) {
    ...
    if (that.data.isWm==0){
      ...
      var newmap = wx.getStorageSync(that.data.storeId + 'wmNewmap');
    }
    if (!newmap) {
  ...
  loadata: function () {
    ...
    config.post('wxApi/s/menuV3', { version: version!=undefined ? version:'' }, function (ret) {
      ...
      if (that.data.isWm) {
        ...
        var newmap={};
        wx.removeStorageSync(that.data.storeId + 'newmap');
        wx.removeStorageSync(that.data.storeId + 'wmNewmap');
        wx.removeStorageSync(that.data.storeId + 'menu');
